...
public class ExcelFormatOut02 implements ExcelFormatOut_I {
	...
	private void initSummaryCell(CellDes[] summaryCellDes,String[] summaryWriteVal,String[] summary){
		...
		summaryWriteVal[0]="已完成出押总台帐";//报表标题
		summaryCellDes[0].colStart=2;
		summaryCellDes[0].colEnd=6;							//合并3到7单元格
		...
		summaryCellDes[0].fontSize=20;						//使用22号字
		...
		summaryWriteVal[6]=summary[1];						//开始查询时间段
	...
	private CellDes[] initColCellDes(int colCount){
		...
		colCellDes[6].textFormat=TextF.millSpt;		//质押券面额 显示千分位
		colCellDes[7].textFormat=TextF.millSpt;		//敞口金额 显示千分位
		colCellDes[6].decDigits=4;					//质押券面额（万元）  4位小数
		colCellDes[7].decDigits=6;					//敞口金额（万元） 6位小数
		colCellDes[6].align="R";  					//质押券面额（万元）右对齐
		colCellDes[7].align="R";  					//敞口金额（万元）右对齐
...
//		colCellDes[17].useFormat=false;				//打印实参cdc#PRP100810020100中比形参多了"调整方向"一项,有问题,这里将多的这一项设为不显示内容
	...
	private void initTableList(ArrayList<String[]> tableList,String[] tableListLable,String[] summary,ArrayList<String[]> writeList){
		...
		tableListLable[0]=summary[4];			//列名 业务发生日期,从实参的标题取值组中获得
		...
		tableListLable[9]=summary[5];			//列名 到期日期 ,从实参的标题取值组中获得
		...
		for (int i = 0; i < tableList.size(); i++) {
			...
			writeLine[5]=aLine[3];				//出押债券
			writeLine[6]=aLine[4];				//质押券面额（万元）
			writeLine[7]=aLine[5];				//敞口金额
	...
	private void initSumCellDes(CellDes[] sumCellDes,String[] sumWrite,String[] sumVal){
		...
		sumWrite[6]=sumVal[0];//质押价值合计 	第7列 对应数组下标为6
		sumWrite[7]=sumVal[1];//敞口金额合计	    第8列
		
		//显示千分位
		sumCellDes[6].textFormat=TextF.millSpt;
		sumCellDes[7].textFormat=TextF.millSpt;
		
		sumCellDes[6].decDigits=4;//质押价值合计 4位小数 
		sumCellDes[7].decDigits=6;//敞口金额合计	6位小数 
		sumCellDes[6].align="R";  //右对齐
		sumCellDes[7].align="R";  //右对齐
