...
public class ExcelFormatOut09 implements ExcelFormatOut_I{
	...
	private void initSummaryCell(CellDes[] summaryCellDes,String[] summaryWriteVal,String[] summary){
		...
		summaryWriteVal[id]="管理中受押分类台账（按业务类别）";	//报表标题
		...
		id=id+1;
		summaryWriteVal[id]="受押方债券账户简称：";				//出押方债券账户简称 标签
		...
		id=id+1;
		summaryWriteVal[id]="受押方债券账号：";					//出押方债券账号 标签
		...
		id=id+1;
		summaryWriteVal[id]=summary[4];							//业务类别
		summaryCellDes[id].colStart=3;
		summaryCellDes[id].colEnd=3;
		summaryCellDes[id].rowStart=3;
		summaryCellDes[id].rowEnd=3;
	...
	private CellDes[] initColCellDes(int colCount){
		...
		colCellDes[16].textFormat=TextF.date;			//到期日/完成日  需要转换格式
		colCellDes[16].dateFormat=TextF.yyyyNMMYddR;		//日期转换格式
		...
		colCellDes[9].decDigits=6;					//债券质押率（%） 6位小数
		colCellDes[10].decDigits=6;					//质押价值（万元）6位小数
		colCellDes[11].decDigits=6;					//敞口金额（万元） 6位小数
		colCellDes[12].decDigits=6;					//差值（万元） 6位小数
	...
	private void initTableList(ArrayList<String[]> tableList,String[] tableListLable,String[] summary,ArrayList<String[]> writeList){
		...
		tableListLable[2]=summary[5];				// 业务发生日期,需要从实参的标题取值组中获得
		...
		tableListLable[16]=summary[6];				// 到期日期 ,需要从实参的标题取值组中获得
		...
		for (int i = 0; i < tableList.size(); i++) {
			...
			writeLine[0]=aLine[17];				//业务类别
			writeLine[1]=aLine[3];				//业务名称
			writeLine[2]=aLine[0];				//业务发生日期
			writeLine[3]=aLine[1];				//合同号
			writeLine[4]=aLine[2];				//出押方
			writeLine[5]=aLine[4];				//受押债券
	...
	private void initSubTotalDes(CellDes sumTotalDes){
		...
		sumTotalDes.sumCol=new int[]{10,11,12};			//参与小计的列
